...
class Index extends Cmsbase
{
    ...
    public function map(){
        ...
        $info = Db::table("yzn_article")->where("id",$id)->find();
        if(empty($info)){
            return $this->fetch('/map' );
        }
        $local = ['114.011538','22.639863'];
        $desc = explode(PHP_EOL,$info["description"]);
        $api = 'https://restapi.amap.com/v3/geocode/geo?key=82fb2be73d98bd7afc76b8eedb404c2d&city=深圳&address=';
        foreach($desc as $d){
            if(strpos($d,"详细地址") !== false){
                $addr = explode("详细地址",$d);
                if(!isset($addr[1])){
                    continue;
                }
                ...
                $addr = trim($addr,":");
                $addr = trim($addr,"：");
                $rsp = json_decode(file_get_contents($api.$addr),true);
                if($rsp["info"] !== "OK" || !isset($rsp["geocodes"][0]["location"])){
                    continue;
                }
                $local = explode(",",$rsp["geocodes"][0]["location"]);
            }
        }
        $info["description"] = str_replace(["\n","\r\n",PHP_EOL],'<br/>',$info["description"]);
        $this->assign([
            "local"=>$local,
            "title"=>$info["title"],
            "thumb"=>$info["thumb"],
    ...
    public function get(){
        $id = input("id",0,"int");
        $model = input("mode","");
        $allow = ["article"=>"yzn_article_data","picture"=>"yzn_picture_data","product"=>"yzn_product_data"];
        if(!isset($allow[$model])){
            return json(["code"=>200,"msg"=>"获取成功","data"=>""]);
        }
        $info = Db::table($allow[$model])->where("id",$id)->find();
        if(empty($info)){
            return json(["code"=>200,"msg"=>"获取成功","data"=>""]);
        }
